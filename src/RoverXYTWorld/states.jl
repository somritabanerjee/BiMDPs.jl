...
function POMDPs.states(mdp::RoverXYTWorldMDP)
...
function visited_list(mdp::RoverXYTWorldMDP)
    num_tgts = length(mdp.reward_vals)
    return boolean_permutations(num_tgts)
end

function non_null_states(mdp::RoverXYTWorldMDP)
    visited_options = visited_list(mdp)
...
function POMDPs.stateindex(mdp::RoverXYTWorldMDP, s::State)
...
POMDPs.initialstate(mdp::RoverXYTWorldMDP) = GWUniform((mdp.grid_size[1], mdp.grid_size[2], mdp.max_time, length(mdp.reward_vals)))

function boolean_permutations(length::Int)
    return [[((i >> (length - j)) & 1) == 1 for j in 1:length] for i in 0:(2^length - 1)]
end

function rand_state(rng::AbstractRNG, mdp::RoverXYTWorldMDP)
    return State(rand(rng, 1:mdp.grid_size[1]), rand(rng, 1:mdp.grid_size[2]), rand(rng, 1:mdp.max_time), rand(visited_list(mdp)))
end

function test_state_indexing(mdp::RoverXYTWorldMDP)
