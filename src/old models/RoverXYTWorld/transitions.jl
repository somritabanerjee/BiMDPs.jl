function POMDPs.transition(mdp::RoverXYTWorldMDP, s::State, a::Action)
    verbose = false
...
inbounds(mdp::RoverXYTWorldMDP, s::State) = 1 ≤ s.x ≤ mdp.grid_size[1] && 1 ≤ s.y ≤ mdp.grid_size[2] && 1 ≤ s.t ≤ mdp.max_time && s.visited in visited_list(mdp)
