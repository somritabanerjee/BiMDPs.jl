function print_stepthrough(mdp::RoverXYTWorldMDP, policy; init_state = State(1,1,1,fill(false, length(mdp.reward_vals))))
...
function collect_stepthrough(mdp::RoverXYTWorldMDP, policy; init_state = State(1,1,1,fill(false, length(mdp.reward_vals))))
    steps = collect(stepthrough(mdp, policy, init_state, "s,a,r", max_steps=100))
    return steps
end

using Statistics
function run_simulation(mdp::RoverXYTWorldMDP, policy, q_learning_policy, sarsa_policy; N_sim = 10000, max_steps = mdp.max_time)
