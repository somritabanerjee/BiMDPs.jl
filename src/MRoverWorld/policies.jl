...
function policy_grid(policy::Policy, xmax::Int, ymax::Int; t::Int = 1, measured::Vector{Bool}=fill(true,length(policy.mdp.tgts)), visited::Vector{Bool} = fill(false, length(policy.mdp.tgts)))
    arrows = Dict(UP => "↑",
                  DOWN => "↓",
                  LEFT => "←",
                  RIGHT => "→",
    ...
    for x = 1:xmax, y = 1:xmax
        s = MState(x, y, t, measured, visited)
...
function values(mdp::MRoverWorldMDP, policy::Policy)
...
function values(mdp::MRoverWorldMDP, policy::Union{NothingPolicy, FunctionPolicy})
    valid_states = non_null_states(mdp)
    rewards = map(s->reward(mdp, s), valid_states)
end

function values(mdp::MRoverWorldMDP, policy::ValuePolicy)
...
function one_based_policy!(policy)
    ...
    println("policy is nothing policy: $(policy isa NothingPolicy)")
    if all(iszero, policy.policy)
        policy.policy[:] = ones(eltype(policy.policy), length(policy.policy))
    end
end

function solve_using(solver_name::String, mdp::MRoverWorldMDP; vi_max_iterations=50, q_n_episodes=500, sarsa_n_episodes=500)
    if solver_name == "vi"
